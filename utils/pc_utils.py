...
Originally from : https://github.com/liruihui/PU-GAN/blob/master/Common/pc_util.py
...
# Uneven downsampling of point clouds
# The parameter k will control the minimum distance of the generated point cloud. 
# The smaller the k, the lower the randomness of the point cloud and the more uniform 
# (the minimum value is 1, at this time, it becomes uniform sampling)
...
if __name__ == "__main__":
    ...
    # points = read_ply("data/reconstruction/bunny_data/00.ply")[:, 0:3]

    # gt_point = downsample_points(points, 5000)
    # save_ply(gt_point, "data/reconstruction/bunny_data/00_5000.ply")
    


    points = read_ply("data/experimental data/model/german-shepherd-dog-3d-print-model/noise/model2000r1.ply")[:, 0:3]
    norm_point, centroid, furthest_distance = normalize_point_cloud(points)
    jit_point = jitter_perturbation_point_cloud(np.expand_dims(norm_point, 0), sigma=0.50, clip=1)
    jit_point = np.squeeze(jit_point, 0)
    save_ply(norm_point, 'data/experimental data/model/german-shepherd-dog-3d-print-model/noise/model_50.ply')